$(document).ready(function(e) {
  ...
  var pageNaviConf = {
    homepageUrl : "http://www.loichucvn.com",
    postPerPage: getMaxResult("max-results") || 9,
    numOfPages: 5,
    firstText: "Đầu tiên",
    lastText: "Cuối cùng",
    nextText: "»",
  ...
  function getCurrentPageByEntryPosition(data, currentUpdatedMax) {
    ...
    for(var pos = 0; pos < data.feed.entry.length; pos ++) {
      ...
      if(currentUpdatedMax === publishedTime) {
        currentPage = getNumOfPage(pos+2);
        break;
      }
    }
    return currentPage;
  ...
  function generateDOM(pageLink, totalPage, currentPage) {
    ...
    if(currentPositionDisplay > 1) {
      html += '<a href="' + pageLink[0]+'">'+pageNaviConf.firstText+'</a>';
    }
    
    if(currentPage > 1) {
      html += '<a href="'+pageLink[currentPage-2]+'">'+pageNaviConf.prevText+'</a>';
    }
    ...
    if(currentPage < totalPage) {
      html += '<a href="'+pageLink[currentPage]+'">'+pageNaviConf.nextText+'</a>';
    }
    
    if(lastPage < totalPage) {
      html += '<a href="'+pageLink[totalPage -1]+'">'+pageNaviConf.lastText+'</a>';
    }
    ...
    for (var i = 0; i < rcp_numposts; i++) {
        ...
        for (var j = 0; j < entry.link.length; j++) {
            if ('thr$total' in entry) {
                ...
                rcp_commentsNum = Math.floor((Math.random() * 2000) + 1);
            };
